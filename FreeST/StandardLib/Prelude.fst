...
-- -- | end.
-- accept : forall a : 1A . *!500a ->[top,bot] dualof a
-- accept c =
--   let (x, y) = new @a () in
--   send x c;
--   y
...
-- -- |   c |> send "Hello!" |> wait
-- -- | ```
-- forkWith : forall a:1A b:*T . (dualof a 1->[top,bot] b) ->[top,bot] a
-- forkWith f =
--   let (x, y) = new @a () in
--   fork (\_:() 1-> f y);
--   x
...
-- -- | ```
-- runServer : forall a:1A b:*T . (b ->[top,bot] dualof a 1->[top,bot] b) ->[top,bot] b ->[top,bot] *!600a ->[top,bot] Diverge
...
-- type OutStream : 1S = +501{ PutChar : !502Char ; OutStream
--                        , PutStr  : !503String ; OutStream
--                        , PutStrLn: !504String ; OutStream
--                        , SWait   : Wait 505
--                        }

-- -- | Unrestricted session type for the `OutStream` type.
-- type OutStreamProvider : *A = *?506OutStream

-- -- | Closes an `OutStream` channel endpoint. Behaves as a `close`.
-- hCloseOut : OutStream ->[top,bot] ()
-- hCloseOut c = c |> select SWait |> wait

-- __hGenericPut : forall a:*T . (OutStream ->[top,bot] !507a ; OutStream) ->[top,bot] a ->[top,bot] OutStream ->[top,bot] OutStream
-- __hGenericPut sel x outStream = sel outStream |> send x

-- -- | Sends a character through an `OutStream` channel endpoint. Behaves as 
-- -- | `|> select PutChar |> send`.
-- hPutChar : Char ->[top,bot] OutStream ->[top,bot] OutStream
...
-- hPutStr : String ->[top,bot] OutStream ->[top,bot] OutStream
...
-- hPutStrLn : String ->[top,bot] OutStream ->[top,bot] OutStream
...
-- hPrint : forall a:*T . a ->[top,bot] OutStream ->[top,bot] OutStream
-- hPrint x = hPutStrLn (show @a x)

-- __hGenericPut_ : forall a : *T . (a ->[top,bot] OutStream ->[top,bot] OutStream) ->[top,bot] a ->[top,bot] OutStreamProvider ->[top,bot] ()
...
-- hPutChar_ : Char ->[top,bot] OutStreamProvider ->[top,bot] ()
...
-- hPutStr_ : String ->[top,bot] OutStreamProvider ->[top,bot] ()
...
-- hPutStrLn_ : String ->[top,bot] OutStreamProvider ->[top,bot] ()
...
-- hPrint_ : forall a:*T . a ->[top,bot] OutStreamProvider ->[top,bot] ()
...
-- type InStream : 1S = +508{ GetChar: ?509Char   ; InStream
--                       , GetLine: ?510String ; InStream
--                       , IsEOF  : ?511Bool   ; InStream
--                       , SWait  : Wait 512
--                       }

-- -- | Unrestricted session type for the `OutStream` type.
-- type InStreamProvider : *A = *?513InStream

-- -- | Closes an `InStream` channel endpoint. Behaves as a `close`.
-- hCloseIn : InStream ->[top,bot] ()
-- hCloseIn c = c |> select SWait |> wait

-- __hGenericGet : forall a:*T . (InStream ->[top,bot] ?514a ; InStream) ->[top,bot] InStream ->[top,bot] (a, InStream)
-- __hGenericGet sel c = receive $ sel c

-- -- | Reads a character from an `InStream` channel endpoint. Behaves as 
-- -- | `|> select GetChar |> receive`.
-- hGetChar : InStream ->[top,bot] (Char, InStream)
...
-- hGetLine : InStream ->[top,bot] (String, InStream)
...
-- hIsEOF : InStream ->[top,bot] (Bool, InStream)
...
-- hGetContent : InStream ->[top,bot] (String, InStream)
-- hGetContent c = 
...
-- __hGenericGet_ : forall a:*T . (InStream ->[top,bot] (a, InStream)) ->[top,bot] InStreamProvider ->[top,bot] a
...
-- hGetChar_ : InStreamProvider ->[top,bot] Char
...
-- -- | enpoint with `hCloseIn`.
-- hGetLine_ : InStreamProvider ->[top,bot] String
...
-- hGetContent_ : InStreamProvider ->[top,bot] String
-- hGetContent_ inp = 
...
-- print : forall a:*T . a ->[top,bot] ()
-- print x = putStrLn $ show @a x

-- -- Internal stdout functions
-- __runPrinter : () ->[top,bot] dualof OutStream 1->[top,bot] ()
...
-- __runErrPrinter : () ->[top,bot] dualof OutStream 1->[top,bot] ()
...
-- getChar : () ->[top,bot] Char
-- getChar _ = hGetChar_ stdin

-- -- | Reads a single line from `stdin`. 
-- getLine : () ->[top,bot] String
-- getLine _ = hGetLine_ stdin

-- -- Internal stdin functions
-- __runReader : () ->[top,bot] dualof InStream 1->[top,bot] ()
