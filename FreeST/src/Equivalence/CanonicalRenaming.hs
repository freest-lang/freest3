...
import qualified Typing.Normalisation as N
import           Bisimulation.Minimal ( minimal )

-- TODO: Labelled, dualof, CForall

-- Already implemetned in Bisimulation/Minimal.hs
...
varRename (T.Forall s (B.Bind sb a k t)) b c
  ...
  | a == c = T.Forall s $ B.Bind sb a' k $ varRename (varRename t a a') b c
    where a' = head $ filter (`Set.notMember` set) freshVars
          set = Set.union (T.free t) (Set.singleton c)
...
first t = head $ filter (`Set.notMember` T.free t) freshVars
...
canonicallyRename t = N.normalise t
