module Equivalence.Subtyping (subtype) where
...
subtype t u = 
  ...
  subG (convertToGrammar [minimal t, minimal u])

-- | Assumes a grammar without unreachable symbols
subG :: Grammar -> Bool
subG g@(Grammar [xs, ys] ps) = expand subExpansion queue rules ps
...
subExpansion :: Expansion
subExpansion ps (xs, ys) =
  -- extract transitions
  ...
  where
    ...
    matchTrans ts1 ts2 = Set.fromList $ Map.elems $ Map.intersectionWith (,) ts1 ts2