...
eval _ _ (E.App p (E.Var _ x) l) | x == mkVar p "select" = return
  $ PrimitiveFun (\(Chan c) -> IOValue $ fmap Chan (send (Label (show l)) c))
eval ctx eenv (E.App p (E.Var _ x) e) | x == mkVar p "collect" = eval ctx eenv e
