module Utils.ShowDefault (ShowWithDefault(..)) where
...
showTypeDefault tops (PairType p t u) =
  lookupPos tops p (PairType p (showTypeDefault tops t) (showTypeDefault tops u))
...
showTypeDefault tops t =
  Map.findWithDefault t (position t) tops

lookupPos :: TypeOpsEnv -> Pos -> Type -> Type
lookupPos tops p defaultType =
  case tops Map.!? p of
    Just u  -> u
    Nothing -> defaultType
...
showExpDefault tops (New p t u) =
  New p (showTypeDefault tops t) u
