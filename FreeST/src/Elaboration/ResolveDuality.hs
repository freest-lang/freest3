...
-- Non session-types
solveDual _ t = addError (DualOfNonSession (clearSource (getSpan t)) t) $> t
...
solveDBind solve v (Bind p a k t) =
  Bind p a k <$> solve (Set.insert a v) (subs (T.Dualof (clearSource p) $ T.Var (clearSource p) a) a t)
