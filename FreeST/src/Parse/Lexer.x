...
-- Operators
  ...
  "+."            { \p s -> TokenPlusF (internalPos p s)}
  "-"                           { \p s -> TokenMinus (internalPos p s) }
  "-."      {\p s -> TokenMinusDot (internalPos p s)}
  "*"				{ \p s -> TokenTimes (internalPos p s) }
  "*."      { \p s -> TokenTimesDot (internalPos p s)}
  "^"				{ \p s -> TokenRaise (internalPos p s) }
  "**"      { \p s -> TokenRaiseTimes (internalPos p s)}
  ...
  "^^"      { \p s -> TokenAppendString (internalPos p s)}
  "/"  		                { \p s -> TokenDiv (internalPos p s) }
  "/."                    { \p s -> TokenDivDot (internalPos p s)}
...
-- Basic types
  ...
  Float       { \p s -> TokenFloatT (internalPos p s)}
...
-- Values
  ...
  ([\-\+]?@numspc@decimal"."@decimal@exponent?|@numspc@decimal@exponent) { \p s -> TokenFloat (internalPos p s) (read $ filter (/= '_') s)}
...
internalPos (AlexPn _ l c) s = 
  let startPos = (l, c) in 
  let endPos   = (l, c+(length s)) in
