...
data ParseResult a = Ok a | Failed (Pos, ErrorType)
...
checkDupProgVarDecl x = do
  ...
  case vEnv Map.!? x of
    Just a -> 
      let p = pos x in addError p (MultipleDatatypeDecl (pos a) x p)
    Nothing -> return ()
...
checkDupTypeDecl a = do
  ...
  case tEnv Map.!? a of
    Just (_, s) ->
      addError (pos a) (MultipleTypeDecl (pos a) a (pos s))
    Nothing -> return ()
...
checkDupFunDecl x = do
  eEnv <- getPEnv
  case eEnv Map.!? x of
    Just e -> addError (pos x) (MultipleFunBindings (pos x) x (pos $ snd e))
    Nothing -> return ()
