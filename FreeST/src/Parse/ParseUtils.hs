...
mkSpanFromSpanSpan :: Span -> Span -> ParseState Span
mkSpanFromSpanSpan (Span _ p1 _) (Span _ _ p2) = do
  m <- modulePath
  return $ Span m p1 p2
...
leftSection op e s = do
  ...
  where
    genFstType (T.Arrow _ _ _ _ t _) = t
    genFstType t = t
...
typeListsToUnArrows a = 
  map \(c, ts) -> (c, foldr (T.Arrow (getSpan c) Un T.Top T.Bottom) (T.Var (getSpan a) a) ts)
