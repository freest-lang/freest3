...
synthetise' _ kEnv (T.Var p a) = case kEnv Map.!? a of
  Just k -> return k
  Nothing -> addError p (TypeVarNotInScope p a) $> omission p
-- Type operators
...
checkContractive s a t = let p = pos t in
  unless (contractive s a t) $ addError p (TypeNotContractive p t a)
...
checkAgainst' s kEnv expected t = do
  ...
  S.when (not $ actual <: expected)
    (addError (pos t) (CantMatchKinds (pos t) expected actual t))
  $> expected
...
checkAgainstSession' s kEnv t = do
  ...
  S.when (p /= K.Session) $ let p = pos t in
    addError p (ExpectingSession p t k)
