...
messagePair = do
  ...
  return (T.Message pos pol t,
          T.Message pos pol t)

varPair :: Gen (T.Type, T.Type)
varPair = do
  a <- arbitrary
  return (T.TypeVar pos a,
          T.TypeVar pos a)
...
distrib n = do
  ...
  p <- arbitrary
  return (T.Semi pos (T.Choice pos p m1) t,
...
assoc n = do
  ...
  return (T.Semi pos t (T.Semi pos v x),
          T.Semi pos (T.Semi pos u w) y)
...
recRecL n = do
  ...
  return (T.Rec pos xk (T.Rec pos yk t),
...
unfoldt n = do
  ...
  xk <- arbitrary
  return (T.Rec pos xk t,
          Rename.unfold (T.Rec pos xk u'))
...
varSkip = do
  x <- arbitrary
  return (T.TypeVar pos x,
          T.Skip pos)

messageVar :: Gen (T.Type, T.Type)
messageVar = do
  (p, b, x) <- arbitrary
  return (T.Message pos p b,
          T.TypeVar pos x)
