...
type  EStack : SL = &{Push: ?Int; NEStack; EStack,  End: Skip}
type NEStack : SL = &{Push: ?Int; NEStack; NEStack, Pop: !Int}

-- Stack server. The empty stack case
eStack : forall a:SL . EStack;a -> a
eStack c =
  match c with {
...
neStack : forall a:SL . Int -> NEStack;a -> a
neStack x c =
  match c with {
...
pushE : forall a:SL . Int -> dualof EStack ; a -> dualof NEStack ; dualof EStack ; a
pushE n c = select Push c & send n

-- Stack operations. Push on a nonempty stack
pushNE : forall a:SL . Int -> dualof NEStack ; a -> dualof NEStack ; dualof NEStack ; a
pushNE n c = select Push c & send n

-- Stack operations. Pop from a nonempty stack (and print the result)
pop : forall a:SL . dualof NEStack;a -> a
pop c = 
...
reverseNE : forall a:SL . Int -> dualof NEStack ; a -> dualof NEStack ; a
reverseNE n c =
  if n == 0
  then c
  else
