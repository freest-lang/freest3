...
repetidor i n
    | i == 0    = Nil
    | otherwise = Cons n (repetidor (i-1) n)

-- makeUntil
makeUntil : Int -> IntList
makeUntil i | i == 0 = Nil | otherwise = Cons i (makeUntil (i-1)) 

-- takeOutNumber
takeOutNumber : IntList -> Int -> IntList
takeOutNumber Nil        n = Nil
takeOutNumber (Cons x y) n
    | x == n    = takeOutNumber y n
    | otherwise = Cons x (takeOutNumber y n)
...
equalsList Nil        _   = False
equalsList (Cons a b) Nil = False
equalsList (Cons a b) (Cons c d)
    | a == c    = equalsList b d
    | otherwise = False
