...
-- |Receive a value on a shared channel and return it
receive_ : forall a:SL . *?a -> a
receive_ c = fst [a, *?a] $ receive c
...
accept sharedChannel =
  let (x, y) = new a in -- Create a new linear channel
  send x sharedChannel; -- send one end to client
  y                     -- return the other

-- 3 _ Types for the FTP server
...
init n pid =
  ...
  fork $ parallel [Diverge] n (\ _:() -> ftpThread state w);
  ftpd pid r
...
actions : State -> dualof FTPSession -> FTPThread -o Diverge
actions state s b =
  match s with
    { Get s ->
