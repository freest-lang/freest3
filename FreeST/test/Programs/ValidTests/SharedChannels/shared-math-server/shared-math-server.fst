...
-- type Promotion : SU = *?(String, CreditCard, Int)
type Promotion  : SU = *?Promotion'
type Promotion' : SL = !String; !CreditCard; !Int

type Decision : SL = &{ Accepted: ?Promotion
                      , Denied  : ?String
                      }

type DonationS : SU = rec a:SU . !(Donation); a
type Donation : SL = +{ SetTitle: !String; Donation
...
helpSavingTheWolf donationServer =
    ...
    match p with {                                              -- wait for the outcome
        Accepted p ->                                           -- if accepted, we have three benefactors
            let (prom, _) = receive p in
            fork $ donate prom "Benefactor1" "2345" 5;
            fork $ donate prom "Benefactor2" "1234" 20;
            fork $ donate prom "Benefactor3" "1004" 10,
...
-- setup : dualof Donation -> String -> Int -> ()
-- setup p title date =
--     match p with {
...
--                       else 
--                         let _ = select Accepted p & promotion in ()
--     }
...
--     fork $ donationServer ps1;          -- send one end to the Donation Server
--     fork $ helpSavingTheWolf ps2;       -- let the whole world know the other
--     fork $ helpSavingTheWolf ps2;
--     helpSavingTheWolf ps2

{-
*?(rec a:SL . +{Commit: &{Accepted: *?(!String ; !String ; !Int), Denied: ?String}, SetDate: !Int ; a, SetTitle: !String ; a})
*?(rec b:SL . +{Commit: &{Accepted:  ?(!String ; !String ; !Int) ; *!(rec b:SL . +{Commit: &{Accepted: *?(!String ; !String ; !Int), Denied: ?String}, SetDate: !Int ; b, SetTitle: !String ; b}), Denied: ?String}
    , SetDate: !Int ; a
    , SetTitle: !String ; a}
    )
-}