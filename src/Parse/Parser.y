...
--  | choice Expr with '{' ChoiceMap '}'       { Match (position $1) $2 $5 }
...
-- Choice
--   : ProgVarWildSeqPair ':' Label {  }
  
ProgVarWildSeqPair :: { Expression }
  : ProgVar   { ProgVar (position $1) $1 }
  | Pair      { $1 }
...
--  | ChoiceMap                        { Choice defaultPos $1 }-- (position $1) $1 }
...
ChoiceMap :: { ChoiceMap }
  : ChoiceField               { uncurry Map.singleton $1 }
  | ChoiceField '+' ChoiceMap { uncurry Map.insert $1 $3 }
...
Label :: { Label }
 : Polarity ProgVar    { ChoiceLabel (snd $1) $2 }
 | Polarity BasicType  { MessageLabel (snd $1) (snd $2) }
 | TypeVar             { VarLabel $1 }
