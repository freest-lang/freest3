...
\subsection{Checking the bisimilarity of simple grammars}
...
\begin{align*}
  &\simplify(\mathit{na}, q, \productions) = \expand(\Fold(\mathsf{append}, q,
    \Fold(\mathsf{apply} \; \productions, \mathit{na}, \mathsf{rules})))
\end{align*}
...
Given two context-free session types $S$ and $T$, function $\BisimT$
decides the equivalence of $S$ and $T$. This function determines the
start words for $S$ and $T$ by first translating $S$ to a grammar
and enriching the latter with productions for $T$. After pruning the productions 
in the grammar (using function $\Prune$), the equivalence of the given types
is decided using function $\Bisim$.

\begin{align*}
  &\BisimT(T, U) = \Bisim(\vec X, \vec Y, \productions'')\\
    &\begin{array}{ll}
  	\text{where: } & (\vec X, \productions) = \grm (S, \emptyset)\\
  	& (\vec Y, \productions') = \grm (T, \productions)\\
  	& \productions'' = \Prune (\productions')
  \end{array}
\end{align*}
