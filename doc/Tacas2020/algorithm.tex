...
\begin{equation*}
  ...
  \grm(T,\productions) = (\word(T),\; \productions_1) \\
  %
\end{equation*}
\[
  \text{ where }\left\{
  \begin{array}{lll}
  	\productions_{n+1} &= \productions\\
  	\productions_i &= \productions_i' \cup \{X_i \rightarrow a_j\vec Y_j \vec Z\},
  	\text{ with }(z \vec z, \productions_i')=\grm(\Unravel(T_i'), \productions_{i+1})
  \end{array}\right.\\\\
\]
