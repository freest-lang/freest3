...
We use this fact to prune out unreachable symbols in unnormed words.
...
\begin{example}
  \label{ex:prune}
  Recall Example~\ref{ex:productions} and notice that $X_1 X_2$ and
  $Y_1 Y_2$ are both unnormed. We can easily see that the last
  occurrence of $X_2$ in the last production for $S$ is unreachable \vv{qual?},
  hence we can simplify it and obtain the pruned productions for $S$
  \vv{e para T?}
  as:
  %
  \begin{align*}
...
\paragraph{Building expansion trees}
