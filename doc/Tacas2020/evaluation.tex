...
\begin{theorem}[Properties of type bisimilarity]
\label{thm:axioms}
  \begin{enumerate}
    % Congruence
...
% We consider the size of a pair of types as the maximum number of nodes
% in the syntax tree, chosen among the two types.
...
\centering
  \subfloat[Distribution of the execution time per variant]{
    \includegraphics[width=0.75\textwidth]{img/distribution_boxplot}
    \label{fig:distribution}}
  
  \subfloat[Number of timeouts per variant]{ 
    \includegraphics[width=.51\textwidth]{img/timeouts}
    \label{fig:timeouts}}
  %
  \subfloat[Runtime of B1 per number of nodes]{
    \includegraphics[width=.45\textwidth]{img/nodes_time}
...
8.6.5).
% We also implemented the optimisations 1-4 above.
...
Figure~\ref{fig:timeouts} shows the number of timeouts for each
variant.  The base case, B, has 146 positive tests whose execution
time exceeds 2 minutes. 
%Some variants perform better than the base
...
%bring.  For the negative tests, we do not have timeouts. 
...
than all others, presenting a median of 1.4 milliseconds for the positive 
tests and 7 timeouts. By implementing optimization 1, the number of 
timeouts reduced
95\%. The remaining positive tests take, on 
average, 1863.38 ms to run with the base algorithm and 195.68 ms  
with variant B1, 
resulting in an 89\% reduction in the execution time. 
This is the variant in production for the FreeST 
