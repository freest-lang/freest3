...
%	on the structure of $\LTSderives$. 
	...
	\begin{itemize}
		...
		\item Case $\sharp B$: if $\sharp B \LTSderives[\sharp B] \skipk$, then 
			$\word(S) \LTSderivesP[\sharp B] \varepsilon$.
		\item Case $\star l_i$: if
		$\star\{l_i\colon S_i\}_{i\in I} \LTSderives[\star l_i] S_i$, then
...
Lemmas~\ref{lemma:terminated-togrammar},
\ref{lem:transitions_mimic}, and~\ref{lem:transitions_words}. 
