...
%
\begin{proof}
	The direct implication is done by rule induction on the
	hypothesis.
...
%	on the structure of $\LTSderives$. 
	...
	\begin{itemize}
		...
		\item If $\sharp B \LTSderives[\sharp B] \skipk$ then, by definition
		of $\word$, $\word(S) \LTSderivesP[\sharp B] \varepsilon$.
		\item If
		$\star\{l_i\colon S_i\}_{i\in I} \LTSderives[\star l_i] S_i$ then, by 
		definition of $\grm$, 
		...
		\item In the case $S_1;S_2\LTSderives S_2'$, where
		...
		\item If $\mu X.T\LTSderives S'$, then, by recursively going backward 
	...
	\begin{itemize}
		...
		$S \LTSderives[\star l_i] U_j ; T$. \qed
	\end{itemize}
\end{proof}
...
follows by rule coinduction on the LTS. 
