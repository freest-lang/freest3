...
\label{lemma:terminated_session}
	...
	\[S \checkmark \text{ if and only if } 
...
\begin{proof}
	...
	\begin{description}
		...
		session, thus \lstinline{toGrammar S} returns \lstinline{[ ]}.
	\end{description}
	Reciprocally, assume that \lstinline{toGrammar S} returns \lstinline{[ ]} and
	adds no productions. By induction on the structure of S, either:
	\begin{itemize}
		\item $S\triangleq \skipk$ and obviously $S\checkmark$.
		\item $S\triangleq S_1;S_2$, where \lstinline{toGrammar S1} and
			  \lstinline{toGrammar S2} do not add any productions and
			  return \lstinline{[ ]}. Hence, $S_1\checkmark$ and $S_2\checkmark$,
			  which by the \LTS\ implies that $S\checkmark$.
		\item $S\triangleq \mu x. S_1$, where \lstinline{toGrammar (subs (Var y) x t)}
			  does not add any productions and returns \lstinline{[ ]}. Hence, 
