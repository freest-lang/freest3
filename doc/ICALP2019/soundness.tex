...
\[\begin{array}{lll}
    ...
    \begin{array}[t]{l}
    	...
    	\text{$Y_i, P_i = \toGrammarf{S_i}$, $i \in I$ }\\
    ...
    \begin{array}[t]{l}
      ...
      \text{occur in $P$ }\text{ and } Y \rightarrow a_j \vec {Z_j} \in  P\\
      \text{with $j\in J$ are all the transitions from $Y$}\}
    \end{array}\\
\end{array}\]
...
\begin{proof}
	Notice that $Y \rightarrow a \vec Z_j \in  P$ is precisely the first 
...
\begin{proof}
	...
	\begin{itemize}
    	...
    	$\Unravel(S_2)\LTSderives[a] S_2'$. By lemma~\ref{lemma:bisim_unr},
    	since $(S_2, (\vec X_2, P_2))\in \mathcal{R}$ we have 
    	$(\Unravel(S_2), (\vec X_2, P_2))\in \mathcal{R}$ (again, 
    	this is only required when $S_2$ is recursive). 
    	...
    	$(\Unravel(\mu X.S), (X, \{X \rightarrow a_j \vec {Z_j}\vec Y\}_{j\in J} \cup  P))
    	\in\mathcal{R}$. By induction hypothesis on $\Unravel(\mu X.S)$, it follows 
