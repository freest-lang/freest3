...
$Y \rightarrow a\vec Z$~\cite{GNF} (we do not allow productions of the
form $X \rightarrow\varepsilon$). Type variables are the
...
\vv{faz-me especie que as provas nao falem do caso Rec que tanto tempo
  levou a afinar}
...
the length of~$\vec a$.
%
Notice that context-free session types are not necessarily normed,
which precludes using the results by Janc\v ar, Hirshfeld and
...
Contrarily to expansion and the reflexive and congruence
