...
time. Among the different optimisations that we tried, two stand
out:
\begin{enumerate}
...
algorithm computes the \emph{simplest} possible children nodes derived
from $N$. Of course, we need to make sure that a fixed-point exists,
which we do with Theorem~\ref{thm:fixed_point}.
%
...
In the next theorem we prove that the simplification function that consists 
in applying the reflexive, congruence and \BPA\ rules has a fixed point.
Note that the result applies either in the case where all symbols are 
normed as in the case they are not.
...
\label{thm:fixed_point}
	...
	ordered set \upshape{\lstinline|Set (Node, Ancestors)|}, where the set of ancestors
...
Having proved that the fixed point exists, we can now adapt the simplification 
phase to, on the one hand, go through an iteration of the simplification rules 
until reaching the fixed point and, on the other hand, identify promising nodes 
and prepend them rather than appending. An improved version of the 
\lstinline|simplify| function proposed in listing~\ref{lst:algorithm} is 
thus presented in listing~\ref{lst:enhanced}.

The optimisations we propose aim to improve the performance of the algorithm, 
however the branching nature of the expansion tree promotes an exponential 
complexity. Indeed, each simplification step (potentially) generates a polynomial 
number of nodes, each of which with linear size on the size of the input.
In its turn, the same simplification phase may, in 
the worst case, be iterated a linear number of times in the size of the input.
For these reasons the complexity turns out to be exponential.
