...
Most of these papers provide no actual practical algorithm that
can be readily extracted, except the one by Czerwinski and Lasota where 
they provide a polynomial time 
algorithm that decides the bisimilarity of normed context-free processes
in $\mathcal{O}(n^5)$~\cite{czerwinski2010fast}.
%
However, context-free session types are not necessarily normed,
which precludes reducing to this algorithm, or using
the original result by Baeten, Bergstra, and
...
On the other hand, the complexity estimates for bisimilarity in
BPA process are not promising. Kiefer have provided an EXPTIME lower bound for 
BPA bisimilarity by proving this problem is EXPTIME-hard~\cite{kiefer2013bpa}, 
whereas Jan\v car have provided a double exponential upper bound for this
problem and proved that its complexity is 
