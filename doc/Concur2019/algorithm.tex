...
$X_S \rightarrow \,\initialProd\,\vec Y$ in the set of
productions, where $\vec Y$ is the word obtained by running 
...
\begin{example}
  \label{ex:prune}
  Recall Example~\ref{ex:productions} and notice that $X_S$ and $Y_T$
  are both unnormed. We can easily see that the last occurrence of
  $X_4$ in the last production for $S$ is unreachable. Hence, by
  pruning the productions for $S$ we get:
  %
  \begin{center}
    \begin{tabular}{l l l}
...
into a grammar. Children nodes are obtained by expansion from its
...
bisimilar t u = expand ([x], [y]) (prune p)
...
expand p = expand' (Queue.singleton (Set.singleton p, Set.empty))
  ...
  expand' q ps
    ...
    | otherwise   = case expandNode ps n of
        Nothing -> expand (Queue.dequeue q) ps
