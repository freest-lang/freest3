...
\subsection{The two bisimulations are bisimilar}

We have defined a bisimulation relation on types 
$\TypeEquiv \enspace\subseteq \enspace\LTSTypes \times \LTSTypes$
and a bisimulation relation on productions 
$\ProdEquiv \enspace\subseteq \enspace\LTSProds \times \LTSProds$.
Our intent in this subsection if to prove that these bisimulations
are bisimilar and, thus, the bisimulation over types coincide with 
the bisimulation over non-terminal symbols in the grammar.
...
For the purpose of proposing a bisimilarity between the bisimulations over
context-free session types and over non-terminal symbols in the grammar, 
...
\begin{proof}
  ...
  $\reck$  and for
  sequential composition follow by induction and the fact that
...
To prove that the bisimulations $\TypeEquiv$ and $\ProdEquiv$ are 
bisimilar, consider the {\it bridge bisimilarity} 
$\BridgeEquiv \subseteq (\LTSTypes \cup \LTSProds) \times
						(\LTSTypes \cup \LTSProds)$
as the greatest equivalence relation such that:
\begin{equation}
	S \BridgeEquiv X \text{ if } (S, (X,P))\in \mathcal{R}
	\text{ for some productions $P$},
\end{equation}
and where $\mathcal{R}$ is the relation given by:
%on context-free session types and
...
\label{thm:cfst_vs_grammar}
	$S \BridgeEquiv \vec X_S$, for all $S$ and where 
	$ \vec X_S, P_S = \toGrammarf{S}$.
\end{theorem}

\begin{proof}
	To prove this theorem, we only need to prove that
	...
	\begin{itemize}
		...
		transition.
		\item $(\sharp B,(X,\{X \rightarrow \sharp B\}))$: we have
...
\begin{corollary}
\label{corol:composing_with_bridge}
	We have:
	\begin{itemize}
		\item $\left.(\BridgeEquiv^{-1} \cdot \TypeEquiv \cdot \BridgeEquiv)
			\right|_{\LTSProds} \subseteq \enspace \ProdEquiv$
		\item $\left.(\BridgeEquiv \cdot \ProdEquiv \cdot \BridgeEquiv^{-1})
			\right|_{\LTSTypes} \subseteq \enspace \TypeEquiv$
	\end{itemize}
\end{corollary}
\begin{proof}
	\am{TODO}
...
\end{corollary}
\begin{proof}
	Use Theorem~\ref{thm:cfst_vs_grammar} and 
